...
- **MCP prompts and resources are proxied**, not just tools. Prompts are namespaced `<serverId>__<name>` like tools; resources keep their URI unchanged, because the URI is meaningful to the client and there is no namespacing scheme the spec blesses — the registry remembers the owner instead. All three go through one policy namespace (namespaced tool name, namespaced prompt name, resource URI), so a rule written for a tool cannot be sidestepped by asking for the same content as a resource.
- **Spring Boot 3.x is supported.** Nothing but the Actuator health indicator depends on a Boot-4-only API, and that autoconfiguration is guarded by `@ConditionalOnClass`, so it is skipped rather than exploding. `compat/boot3` boots the published starter on Boot 3.5, drives it with a real MCP client and runs in CI on every push; `docs/spring-boot-3.md` has the two-line dependency change and what is lost.
- **Published latency benchmarks** (`docs/benchmarks.md`) — +0.30 ms at p50 over calling the upstream directly, with policy, quotas and audit all on. The harness is `GatewayOverheadBenchmark`, excluded from the normal build behind `-Dmcp.benchmark=true` so CI-runner noise is never presented as fact.
- **Configurable circuit breaker** (`mcp.gateway.circuit-breaker.failure-threshold`, `open-duration`).
...
- `ToolVisibility` is a name predicate (`isVisible(principal, capabilityName)`) rather than a `List<Tool>` filter, so tools, prompts and resources can all share it.
